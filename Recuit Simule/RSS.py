...
class RSS:
    ...
    def __init__(self, T0, Tmin, tau, filepath):
        ...
        self.v = np.zeros([self.N, self.N]) #variance
        self.quantileF = np.zeros([self.N, self.N])
        self.X = np.zeros([self.N, self.N]) #chemin parcouru en matrice
        self.alpha = 0.75
        self.taux_maj = 0.2
        ...
        for i in range(self.N - 1):
            for j in range(self.N - 1):
                if i == j:
                    self.v[i][j] = 0;
                if i < j:
                    self.v[i][j] = np.random.random_sample() * self.d[i][j]
                    self.v[j][i] = self.v[i][j]
                    self.quantileF[i][j] = self.d[i][j] + (self.v[i][j] * (np.sqrt(3)/np.pi) * np.log(self.alpha/(1-self.alpha)))
        ...
        self.Chemin()
        if not self.Contrainte_X():
            print("error X")

        E= self.Calcul_Energie()
    ...
    def Chemin(self):
        ...
        for i in range(self.N):
            if i == self.N-1:
                x[self.ordre_trajet[i]][self.ordre_trajet[0]] = 1
            else:
                x[self.ordre_trajet[i]][self.ordre_trajet[i+1]] = 1
        self.X = x

    # Vérifie la contrainte "doit parcourir toutes les villes qu'une seule fois"
    def Contrainte_X(self):

        Xt = np.matrix.transpose(self.X)
        b = True
        for i in range(self.N-1):
            x = np.ndarray.sum(self.X[i])
            y = np.ndarray.sum(Xt[i])
            if not (x == 1 or y == 1):
                b = False
                print(x)
                print(y)
    ...
    # Vérifie la contrainte stochastique
    def Contrainte_4(self):

        s = 0
        for i in range(self.N - 1):
            for j in range(self.N - 1):
                s += self.X[i][j] * self.quantileF[i][j]

        return s > 8361 * (1 + self.taux_maj)
    ...
    def solve(self):
        ...
        while (T > self.Tmin) or (self.Contrainte_4()):
        ...
        # Entre Derniere ville et ville de depart
        plt.plot([self.x[self.ordre_trajet[-1]], self.x[self.ordre_trajet[0]]],
                 [self.y[self.ordre_trajet[-1]], self.y[self.ordre_trajet[0]]], 'k')
        plt.plot(self.x, self.y, 'bo')
